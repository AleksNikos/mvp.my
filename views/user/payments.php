...
<?php
$currentYear = date("Y", $maxTime);
?>
<table id="<?=$currentYear?>" style="display: block;" class="table_info tabcontent">
    <tr class="table_title">
        <th class="date">Date</th>
        <th class="status">Status and ID</th>
        <th class="engines">Engines</th>
...
<!--                <td>Jun 15, 2019, 00:00</td>-->
                ...
                <td class="status <?=$payment["status"]?"green":"red" ?>"><?=$payment["random_string"]?></td>
<!--                <td class="status green">46279279SO</td>-->
                <?php
                $engines = 0;
                $fd="";
                $er="";
                    if($payment["fd_price"]>0){
                        $fd = "Fd";
                        $engines++;
                    }
                    if($payment["er_price"]>0){
                        $er = ", Er";
                        $engines++;
                    }

                    if ($engines>0){
                        echo "<td><?=$engines?>: <span><?=$fd.$er?></span></td>";
                    }else{
                        echo "<td>test</td>";
                    }
                ?>
            ...
            <?php
            if($payment["er_price"]>0 or $payment["fd_price"]>0){
                ?>
                ...
                <tr class="table-row hidden-panel">
                    ...
                    <td>
                        <?=$payment["fd_price"]>0?"$".$payment["fd_price"]:"" ?>
                        <br />
                        <?=$payment["er_price"]>0?"$".$payment["er_price"]:"" ?>
                    </td>
                </tr>
...
<div id="edit-paymant">
    ...
    <?php
        myAjaxWidget::begin([
                "submitSelector" => ".remove",
                "formSelector" => ".edit-payment-form",
                "link"=>"/user/delete-payment-method",
    ...
    <?$form = ActiveForm::begin([
        ...
        <div class="checkboxes">
            <label for="card" class="card">
                ...
                <input id="card" class="hidden" type="radio" value="card" checked name="pay-method-edit">
        ...
        <div class="checkbox">
            <div class="inp-wr">
                <label class="understand">
                    <input name="understand" value="yes" type="checkbox">
...
<!--    <form class="add-payment-form">-->
        <?php
        myAjaxWidget::begin([
                "formSelector" => ".add-payment-form",
                "formClean" => false,
            "afterMethod" => "setWarnings('.add-payment-form')",
            "successMethod" => "ajaxSuccess()"
        ]);
//            Pjax::begin();
            $form = ActiveForm::begin([
        ...
        <div class="checkboxes">
            <label for="add-card" class="card">
                ...
                <span class="name">Card</span>
                <input id="add-card" class="hidden" type="radio" value="card" checked name="Card[payment_method]">
