<?php
/* @var $model app\models\Keys */
use app\widgets\myAjaxWidget;

/* @var $key app\models\Keys */
$this->params['pageID']="keys";
?>
<table class="table_info">
    <tr class="table_title">
        <?php

        ?>
        <th class="key_name"><?=$model?count($model):0?> keys
            <a class="add"data-fancybox data-src="#add-key" href="javascript:;">
                <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path fill-rule="evenodd" clip-rule="evenodd" d="M20 10C20 15.5228 15.5228 20 10 20C4.47715 20 0 15.5228 0 10C0 4.47715 4.47715 0 10 0C15.5228 0 20 4.47715 20 10ZM9.27955 5.36273C9.27996 4.96348 9.60044 4.64299 9.9997 4.64259C10.399 4.64218 10.7188 4.96202 10.7184 5.36128L10.7144 9.27412L14.6197 9.2777C15.019 9.27729 15.3388 9.59713 15.3384 9.99638C15.342 10.196 15.2627 10.3731 15.1307 10.5051C15.0025 10.6333 14.8179 10.7126 14.622 10.7128L10.7092 10.7167L10.7052 14.6296C10.7086 15.0251 10.4482 15.3455 9.98884 15.346C9.58959 15.3464 9.26975 15.0265 9.27016 14.6273L9.27412 10.7144L5.36128 10.7184C4.96202 10.7188 4.64218 10.399 4.64259 9.9997C4.64299 9.60044 4.96348 9.27996 5.36273 9.27955L9.27558 9.27558L9.27955 5.36273Z" fill="url(#paint0_linear)"/>
                    <defs>
                        <linearGradient id="paint0_linear" x1="10" y1="30" x2="30" y2="10" gradientUnits="userSpaceOnUse">
                            <stop stop-color="#6673B4"/>
                            <stop offset="1" stop-color="#9EAEFF"/>
                        </linearGradient>
                    </defs>
                </svg>
            </a>
        </th>
        <th class="key_title">Download</th>
        <th class="key_title">Update</th>
        <th class="key_title">Stop</th>
        <th class="fd">
            <img src="/img/icon-fd.svg" alt="">
            <span>Face detector</span>
        </th>
        <th class="er">
            <img src="/img/icon-er.svg" alt="">
            <span>Em. recogn.</span>
        </th>
    </tr>

    <?php
    $i = 0;
        if($model){
            foreach ($model as $key){$i++;?>

                <tr class="<?=$key->IS_ACTIVATED?"keys green":"keys red" ?>">
                    <td style="text-align: left"><span><?=$i?></span><?=$key->name_key?></td>
                    <td>
                            <a <?=$key->IS_ACTIVATED?'href="/user/download-key?id='.$key->id.'"': ""?>  disabled="<?=!$key->IS_ACTIVATED?"disabled":""?>" target="_blank">
                                <img src="/img/icon-btn-dowld.svg" alt="">
                            </a>
                    </td>
                    <td>
                        <?php
                            if($key->IS_ACTIVATED){

                                myAjaxWidget::begin([
                                    "formSelector"=>"#updateForm".$key->id,
                                    "submitSelector"=>".updateButton",
                                    "successMethod"=>"ajaxSuccess()",
                                    "link"=>"/user/update-key",
                                ]);

                        ?>
                        <form id="updateForm<?=$key->id?>">
                            <input name="id" value="<?=$key->id?>" hidden <?=!$key->IS_ACTIVATED?"disabled":""?>>
                            <a class="updateButton" disabled="<?=!$key->IS_ACTIVATED?"disabled":""?>">
                                <img src="/img/upload.svg" alt="">
                            </a>
                        </form>
                        <?php
                            myAjaxWidget::end();}
                            else{
                                ?>

                                <a class="updateButton" disabled="<?=!$key->IS_ACTIVATED?"disabled":""?>">
                                    <img src="/img/upload.svg" alt="">
                                </a>
                                <?php
                            }
                        ?>
                    </td>
                    <td>
                        <?php myAjaxWidget::begin([
                            "formSelector"=>"#stopForm".$key->id,
                            "submitSelector"=>".stopButton",
                            "successMethod"=>"ajaxSuccess()",
                            "link"=>"/user/stop-key",
                        ])?>
                            <form id="stopForm<?=$key->id?>">
                                <input name="id" value="<?=$key->id?>" hidden>
                                <a class="stopButton">
                                    <img src="/img/stop.svg" alt="">
                                </a>
                            </form>
                        <?php
                        myAjaxWidget::end();
                        ?>
                    </td>
                    <td class="check">
                        <?php
                        if($key->Fd){
                            ?>
                            <svg width="9px" height="7px" viewBox="0 0 9 7" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                                <g id="Symbols" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                                    <g id="ic-check" fill="#D0D8FF">
                                        <path d="M3.2173913,6.65217391 C3.1014487,6.76811652 2.97101522,6.82608696 2.82608696,6.82608696 C2.6811587,6.82608696 2.55072522,6.76811652 2.43478261,6.65217391 L0,4.2173913 L0.782608696,3.43478261 L2.82608696,5.47826087 L8.26086957,1.18571819e-13 L9,0.782608696 L3.2173913,6.65217391 Z" id="ion-ios-checkmark-empty---Ionicons"></path>
                                    </g>
                                </g>
                            </svg>
                            <?php ;
                        }else{
                            ?>
                            <span></span>
                            <?php
                        }
                        ?>
                    </td>
                    <td class="check">
                        <?php
                        if($key->Er){
                            ?>
                            <svg width="9px" height="7px" viewBox="0 0 9 7" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                                <g id="Symbols" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                                    <g id="ic-check" fill="#D0D8FF">
                                        <path d="M3.2173913,6.65217391 C3.1014487,6.76811652 2.97101522,6.82608696 2.82608696,6.82608696 C2.6811587,6.82608696 2.55072522,6.76811652 2.43478261,6.65217391 L0,4.2173913 L0.782608696,3.43478261 L2.82608696,5.47826087 L8.26086957,1.18571819e-13 L9,0.782608696 L3.2173913,6.65217391 Z" id="ion-ios-checkmark-empty---Ionicons"></path>
                                    </g>
                                </g>
                            </svg>
                            <?php ;
                        }else{
                            ?>
                            <span></span>
                            <?php
                        }

                        ?>
                    </td>
                </tr>
                <tr class="d-none hidden-buttons-keys">
                    <td colspan="3">
                        <a href="#">
                            <img src="/img/icon-btn-dowld.svg" alt="">Download
                        </a>
                    </td>
                </tr>
                <tr class="d-none hidden-buttons-keys">
                    <td colspan="3">
                        <a href="#" disabled=disabled>
                            <img src="/img/upload.svg" alt="">Update
                        </a>
                    </td>
                </tr>
                <tr class="d-none hidden-buttons-keys">
                    <td colspan="3">
                        <a href="#" disabled=disabled>
                            <img src="/img/stop.svg" alt="">
                            Stop
                        </a>
                    </td>
                </tr>


            <?php }

        }
    ?>

</table>

